...
(async () => {
    ...
    const id = (node) => {
        let str = `${node.parent ? id(node.parent) + "." : ""}${node.data.name}`;
        return str
    };
    ...
    let json = await d3.json('flare.json');
    let data = hierarchy(json);
    ...
    const chart = () => {
        ...
        svg.append('g')
            ...
            .attr('stroke', (d, i) => color(d3.easeQuad(i / (1 << k) - 1))) // 1 << k = 1 * 2 ** 6 = 64
            .attr('d', d => d.join(''));
